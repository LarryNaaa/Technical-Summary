...
- **未提交读（READ UNCOMMITTED）**
  ...
  - 一个事务还没提交时，它做的变更就能被**别的事务看到**。如果一个事务已经开始写数据，则**另外一个事务则不允许同时进行写操作**，**但允许其他事务读此行数据。**

  - 更新数据时加上**行级共享锁**，事物结束即释放

- **提交读（READ COMMITTED）**

  - 一个事务提交之后，它做的变更才会被其他事务看到 。读取数据的事务**允许其他事务继续访问该行数据，但是未提交的写事务将会禁止其他事务访问该行，会对该写锁一直保持直到到事务提交。**
