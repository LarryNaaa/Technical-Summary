...
- - 用户输入用户名/密码登录，服务端认证成功后，会返回给客户端一个 JWT
  ...
  - 服务端获取请求头 Authorization 中的 JWT，通过base64UrlEncode解密头部和负载，获取JWT信息如签名算法，到期时间，用户信息等。
  - 之后，**服务器使用相同的签名算法对头部，负载和存储在服务器端的私钥重新生成signature，比较生成的和token携带的signature是否一致**，如果一致则合法，允许用户的行为
